...
| `verify_owners` | top | no | Boolean. Before writing, prove with the GitHub API that every owner the ops put into force exists; one that does not is exit 3 and nothing is written (R-41). Needs a token (`--token` or `$GITHUB_TOKEN`). It lives here for the reason `create` does: a guarantee that depends on every call site remembering a flag is not a guarantee. `--verify-owners` may turn it on for a run, never off. |
